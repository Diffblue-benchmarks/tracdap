...
class SqlUserManagerDiffblueTest {
  /**
   * Method under test: {@link SqlUserManager#SqlUserManager(ConfigManager)}
   */
  @Test
  void testNewSqlUserManager() {
    // Arrange
    PluginManager plugins = mock(PluginManager.class);
    when(plugins.createConfigService(Mockito.<Class<IConfigLoader>>any(), Mockito.<String>any(),
        Mockito.<Properties>any())).thenThrow(new EAuthorization("Not all who wander are lost"));
    ...
    // Act and Assert
    assertThrows(EAuthorization.class, () -> new SqlUserManager(new ConfigManager("https://example.org/example",
