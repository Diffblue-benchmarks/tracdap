...
class CompressionInterceptorDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Then return {@link DelayedExecutionListener}.
   * </ul>
   *
   * <p>Method under test: {@link CompressionInterceptor#interceptCall(ServerCall, Metadata,
   * ServerCallHandler)}
   */
  @Test
  @DisplayName(
      "Test interceptCall(ServerCall, Metadata, ServerCallHandler); then return DelayedExecutionListener")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({
    "Listener CompressionInterceptor.interceptCall(ServerCall, Metadata, ServerCallHandler)"
  })
  void testInterceptCall_thenReturnDelayedExecutionListener() {
    ...
    Listener<Object> actualInterceptCallResult =
        compressionInterceptor.interceptCall(call, headers, next);

    // Assert
    verify(delegate).request(1);
    verify(delegate2).request(1);
    verify(delegate).setCompression("gzip");
