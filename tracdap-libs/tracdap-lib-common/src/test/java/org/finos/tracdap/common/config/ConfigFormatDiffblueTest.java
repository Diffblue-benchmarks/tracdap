...
class ConfigFormatDiffblueTest {
   ...
   * </ul>
   *
   * <p>Method under test: {@link ConfigFormat#fromExtension(URI)}
   */
  @Test
   ...
   *
   * <ul>
   *   <li>When Property is {@code java.io.tmpdir} is {@code Unknown config format for file: [%s]}
   *       toUri.
   * </ul>
   *
   * <p>Method under test: {@link ConfigFormat#fromExtension(URI)}
  ...
  @Test
  @DisplayName(
      "Test fromExtension(URI); when Property is 'java.io.tmpdir' is 'Unknown config format for file: [%s]' toUri")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"ConfigFormat ConfigFormat.fromExtension(URI)"})
        ...
        () ->
            ConfigFormat.fromExtension(
                Paths.get(
                        ...
                        "Unknown config format for file: [%s]")
                    .toUri()));
  }
}
