...
class FileVersionValidatorDiffblueTest {
  /**
   * Test {@link FileVersionValidator#fileVersion(FileDefinition, FileDefinition,
   * ValidationContext)}.
   *
   * <p>Method under test: {@link FileVersionValidator#fileVersion(FileDefinition, FileDefinition,
   * ValidationContext)}
   */
  @Test
   ...
   * </ul>
   *
   * <p>Method under test: {@link FileVersionValidator#sameExtension(String, String,
   * ValidationContext)}
   */
  @Test
  @DisplayName(
      "Test sameExtension(String, String, ValidationContext); given 'Field Name'; then calls error(String)")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({
    "ValidationContext FileVersionValidator.sameExtension(String, String, ValidationContext)"
  })
  void testSameExtension_givenFieldName_thenCallsError() {
    // Arrange
    ValidationContext ctx = mock(ValidationContext.class);
    ...
    verify(ctx)
        .error(
            "File extension in field [Field Name] changed between versions: prior = [], new = [.]");
